...
curl -H "X-Github-Event: push" \
    ...
    -X POST \
    -d '{"commits": [{"modified": ["accountservice.yml"]}],"name":"what is this?"}' \
    -ki http://192.168.6.190:8888/monitor
...
docker service create \
    --log-driver=gelf \
    --log-opt gelf-address=udp://192.168.6.190:12202 \
...
docker service create \
    ...
    --log-opt gelf-address=udp://192.168.6.190:12202 \
    --log-opt gelf-compression-type=none \
    --name=vipservice --replicas=1 --network=my_network someprefix/vipservice
